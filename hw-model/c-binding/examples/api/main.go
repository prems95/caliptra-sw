...
type INVOKE_DPE_COMMAND_OUTPUT struct {
	Chksum     uint32
	FipsStatus uint32
	DataSize   uint32
	Data       [DPE_REQ_MAX_SIZE]uint8
}
...
func main() {
	...
	for {
		...
		if C.strstr((*C.char)(unsafe.Pointer(buffer.data)), C.CString("Caliptra RT listening for mailbox commands...")) != nil {
			fmt.Println("**************************************2*********************************")
			var test C.uint32_t
			profileBuffer := C.create_invoke_dpe_command(C.uint32_t(CmdMagic), C.uint32_t(CommandGetProfile), 0)
			fmt.Println("**********************profile buffer $****************************")
			fmt.Println(profileBuffer)
			fmt.Print("Data Content: ")
			for i := 0; i < int(profileBuffer.len); i++ {
				byteValue := *(*uint8)(unsafe.Pointer(uintptr(unsafe.Pointer(profileBuffer.data)) + uintptr(i)))
			...
			var byteSlice []byte
			// Convert the C uint32_t value to a byte slice
			byteSlice = C.GoBytes(unsafe.Pointer(&Check.data[0]), C.int(Check.data_size))

			fmt.Println(byteSlice)

			buffer := bytes.NewReader(byteSlice)

			// Create a GetProfileResp instance to hold the unmarshalled data
			var respHdr RespHdr
			var getProfileRespData GetProfileResp

			// Use binary.Read to unmarshal the binary data into getProfileRespData

			err := binary.Read(buffer, binary.LittleEndian, &respHdr)
			if err != nil {
			...
			err = binary.Read(buffer, binary.LittleEndian, &respStruct)
			if err != nil {
				log.Fatal("Error unmarshalling data:", err)
			}

			// Now you can access and print the fields of getProfileRespData
			fmt.Printf("MajorVersion: %v\n", respStruct.MajorVersion)
			fmt.Printf("MinorVersion: %v\n", respStruct.MinorVersion)
			fmt.Printf("VendorId: %v\n", respStruct.VendorId)
			fmt.Printf("VendorSku: %v\n", getProfileRespData.VendorSku)
			fmt.Printf("MaxTciNodes: %v\n", getProfileRespData.MaxTciNodes)
			fmt.Printf("Flags: %v\n", getProfileRespData.Flags)
			break
		}
	}
